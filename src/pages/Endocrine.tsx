...
import { ActivitySquare, Info, Search, Database, BookOpen, Activity, ArrowRight, Loader2, BrainCircuit } from "lucide-react";
...
const Endocrine = () => {
  ...
  const [pubmedQuery, setPubmedQuery] = useState("Diabetes Mellitus Type 2 Pathophysiology");
  const [pubmedResults, setPubmedResults] = useState<PubMedArticle[]>([]);
  const [isSearchingPubMed, setIsSearchingPubMed] = useState(false);

  const [clinicalQuery, setClinicalQuery] = useState("Diabetes Mellitus");
        ...
        <div className="mb-8 sm:mb-12">
          <div className="flex items-center gap-3 mb-4">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-yellow-500/10">
              <ActivitySquare className="h-5 w-5 text-yellow-500" />
            </div>
            <h1 className="text-2xl sm:text-3xl font-semibold">Endocrine Intelligence</h1>
          </div>
          <p className="text-lg text-muted-foreground max-w-3xl">
            Explore hormonal pathways, metabolism, and pathologies like Diabetes and Thyroid disorders through interactive research data.
          </p>
        </div>
        ...
        <Tabs defaultValue="explorer" className="w-full">
          ...
          <TabsContent value="explorer" className="space-y-6 animate-fade-in">
            <div>
              ...
              <p className="text-muted-foreground mb-6">Structured biomedical knowledge linking endocrine physiology and pathology.</p>
            </div>
            
            <div className="grid gap-6 md:grid-cols-2">
              <Card className="flex flex-col border-border/60 hover:border-yellow-500/30 transition-colors shadow-sm">
                <CardHeader className="pb-3 border-b border-border/40 bg-muted/10">
                  <div className="flex justify-between items-start">
                    <CardTitle className="text-xl text-yellow-600">Type 2 Diabetes</CardTitle>
              ...
              <Card className="flex flex-col border-border/60 hover:border-yellow-500/30 transition-colors shadow-sm">
                <CardHeader className="pb-3 border-b border-border/40 bg-muted/10">
                  <div className="flex justify-between items-start">
                    <CardTitle className="text-xl text-yellow-600">Hashimoto's Thyroiditis</CardTitle>
                    <Badge variant="outline">Pathology</Badge>
                  </div>
                </CardHeader>
                <CardContent className="pt-5 flex-1 space-y-5">
          ...
          <TabsContent value="simulation" className="animate-fade-in">
            <Card className="p-12 border-border/60 shadow-sm flex flex-col items-center justify-center text-center">
              <Activity className="h-12 w-12 text-yellow-500/50 mb-4" />
              <h3 className="text-xl font-semibold mb-2">Endocrine Simulation Under Development</h3>
              <p className="text-muted-foreground">Interactive HPA axis and insulin-glucose feedback loop models are coming in the next update.</p>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="research" className="space-y-6 animate-fade-in">
            <div className="flex gap-4 mb-8 bg-muted/30 p-2 rounded-lg border border-border/50">
              <div className="relative flex-1 flex items-center">
                ...
                <input 
                  type="text" 
                  placeholder="Search endocrine literature..." 
                  className="w-full bg-background border-none rounded-md pl-9 pr-4 py-2 text-sm shadow-sm focus:ring-2 focus:ring-yellow-500 outline-none"
                  value={pubmedQuery}
            ...
            <div className="space-y-4">
              {isSearchingPubMed ? (
                <div className="text-center py-8 text-muted-foreground"><Loader2 className="h-8 w-8 animate-spin mx-auto mb-4 text-yellow-500" /></div>
              ) : (
                pubmedResults.map((paper, i) => (
                  <Card key={i} className="p-5 border-border/60 hover:border-yellow-500/40 hover:shadow-md transition-all cursor-pointer">
                    <h3 className="font-medium text-base text-yellow-600 mb-1">{paper.title}</h3>
          ...
          <TabsContent value="datasets" className="space-y-6 animate-fade-in">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {isSearchingClinical ? (
                <div className="md:col-span-2 text-center py-8 text-muted-foreground"><Loader2 className="h-8 w-8 animate-spin mx-auto mb-4 text-yellow-500" /></div>
              ) : (
                clinicalResults.map((trial, i) => (
                  <Card key={i} className="p-6 flex flex-col h-full border-border/60 hover:border-yellow-500/30 transition-colors shadow-sm bg-gradient-to-br from-background to-muted/20">
