"""
bioquora/nlp/workers/celery_app.py
───────────────────────────────────
...
Usage:
  ...
  celery -A bioquora.nlp.workers.celery_app worker --loglevel=info -c 4

#   # Start with beat scheduler (Step 6):
#   celery -A bioquora.nlp.workers.celery_app beat --loglevel=info
#   celery -A bioquora.nlp.workers.celery_app worker --loglevel=info
"""
...
celery_app.conf.update(
    ...
    task_routes={
        "bioquora.nlp.workers.celery_app.process_paper_task": {"queue": "nlp"},
        "bioquora.nlp.workers.celery_app.process_batch_task": {"queue": "nlp"},
        "bioquora.nlp.workers.celery_app.fetch_pubmed_delta_nightly_task": {"queue": "nlp"},
    },
    beat_schedule={
        "fetch-pubmed-delta-nightly": {
            "task": "bioquora.nlp.workers.celery_app.fetch_pubmed_delta_nightly_task",
...
@celery_app.task(
    bind=True,
    name="bioquora.nlp.workers.celery_app.process_paper_task",
    max_retries=3,
...
def process_paper_task(self, paper_dict: dict, write_to_neo4j: bool = True) -> dict:
    ...
    """
    from bioquora.nlp.models.schemas import ProcessRequest, NLPResult
...
@celery_app.task(
    bind=True,
    name="bioquora.nlp.workers.celery_app.process_batch_task",
    max_retries=1,
...
@celery_app.task(
    bind=True,
    name="bioquora.nlp.workers.celery_app.fetch_pubmed_delta_nightly_task",
    max_retries=1
)
def fetch_pubmed_delta_nightly_task(self, domains: list[str], days_back: int) -> dict:
    ...
    """
    from bioquora.nlp.workers.pubmed_ingest import fetch_pubmed_delta, filter_existing_pmids
    from bioquora.nlp.utils.neo4j_client import Neo4jClient
