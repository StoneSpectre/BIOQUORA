...
from bioquora.nlp.models.schemas import (
    ProcessRequest,
    ProcessResponse,
    BatchProcessRequest,
    BatchJobResponse,
...
from bioquora.nlp.pipeline.model_loader import get_model_registry, ModelRegistry
from bioquora.nlp.pipeline.ner_pipeline import process_text
from bioquora.nlp.utils.neo4j_client import Neo4jClient
...
def process_paper(
    ...
    try:
        ...
        # Step 4: Classify Paper
        paper_classifier = PaperClassifier()
        paper_meta = {
            "title": request.title or "",
            "abstract": request.text or "",
            "mesh_terms": [] # Can be populated from PubMed E-utilities in future
        }
        paper_class = paper_classifier.classify(paper_meta)
        result.paper_classification = paper_class
        
        # Step 3: Extract Relations
        relation_extractor = RelationExtractor(use_mock=True)
        entities_dict = [ent.model_dump() for ent in result.entities]
        raw_relations = relation_extractor.extract_relations(entities_dict, request.text)
        
        # Step 5: Evidence Scoring & Contradiction Detection
        evidence_scorer = EvidenceScorer(neo4j_client=neo4j)
        pmid = request.pmid or f"mock_{uuid.uuid4().hex[:8]}"
        scored_relations = []
        for rel in raw_relations:
            scored = evidence_scorer.process_relation(rel, paper_class, pmid, existing_evidence_count=0)
            scored_relations.append(scored)
            
        result.relations = scored_relations
...
def process_batch(request: BatchProcessRequest):
    ...
    """
    try:
        from bioquora.nlp.workers.celery_app import process_batch_task
...
def batch_status(job_id: str):
    ...
    try:
        from bioquora.nlp.workers.celery_app import get_batch_job_status
